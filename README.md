...
corresponding timestamps vector.
...
Here, all four valid days have 100% of wear time. We hence demonstrate
the `impute_missing_data` method by artificially replacing 1h (4%) of a
valid day with non-wear and running `impute_missing_data` function then.

Function `impute_missing_data` imputes missing data from the “average
day profile”. An “average day profile” is computed as a minute-wise
average of AC across valid days.
